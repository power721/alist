...
func Init(d *gorm.DB) {
	db = d
	err := AutoMigrate(new(model.Storage), new(model.User), new(model.Meta), new(model.SettingItem), new(model.Token), new(model.SearchNode), new(model.TaskItem), new(model.SSHPublicKey))
	if err != nil {
